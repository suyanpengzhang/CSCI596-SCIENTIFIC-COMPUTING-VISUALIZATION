...
/* Main loop over x, y & z directions starts--------------------------*/
  ...
  for (kd=0; kd<3; kd++) {
    ...
    for (kdd=0; kdd<2; kdd++) {
      ...
      /* Send & receive the # of boundary atoms (modified)-----------------------*/
      ...
      /* Send & receive information on boundary atoms (modified)-----------------*/
      
        MPI_Irecv(dbufr,3*nrc,MPI_DOUBLE,MPI_ANY_SOURCE,20,
                 workers,&request);
      
      /* Message buffering (modified)*/
      for (i=1; i<=nsd; i++)
...
----------------------------------------------------------------------*/
  ...
  for (kd=0; kd<3; kd++) {
    ...
    for (kdd=0; kdd<2; kdd++) {
      ...
      /* Send atom-number information (modified)---------------------------------*/  
